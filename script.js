...
function scoreboard() {
  ...
  return `
    <aside class="scoreboard">
      ...
      <div class="score-card"><span>Health</span><strong>${state.health}</strong></div>
      <div class="score-card"><span>Income</span><strong>${state.income}</strong></div>
...
function chooseOption(choiceIndex) {
  ...
  state.health = clampScore(state.health + choice.impact.health);
  state.income = clampScore(state.income + choice.impact.income);
  ...
  document.querySelector("#feedbackArea").innerHTML = `
    <div class="feedback">
      ...
      <button class="primary-button" type="button" id="nextRound">
        ${state.roundIndex === rounds.length - 1 ? "View Final Result" : "Continue"}
      </button>
    </div>
  `;
